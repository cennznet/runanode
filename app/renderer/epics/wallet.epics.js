  ...
  action$.pipe(
    ...
    mergeMap(async ({ payload }) => {
      ...
      if (myWallet) {
        ...
        const syncedWallet = await window[APP_BRAND].api.cennz.syncWalletData(myWallet);
...
const transferEpic = action$ =>
  action$.pipe(
    ...
    mergeMap(async ({ payload }) => {
      ...
      const txHash = await window[APP_BRAND].api.cennz.doGenericAssetTransfer(
        assetId,
        fromAddress,
        toAddress,
        amount,
...
const addAccountEpic = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap(async ({ payload }) => {
      ...
      const { updatedWallet, newAccount } = await window[APP_BRAND].api.cennz.addAccount({ wallet });
      const resolvedWalletItem = R.set(R.lensProp('wallet'), updatedWallet, toUpdateWallet);
      const syncedWallet = await window[APP_BRAND].api.cennz.syncWalletData(resolvedWalletItem);
